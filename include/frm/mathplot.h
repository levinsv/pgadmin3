...
/** @mainpage wxMathPlot
    ...
    <a href="http://wxmathplot.sourceforge.net/screenshot.shtml">Go to the
   screenshots page.</a>
   ...
   wxWindow.
    mpWindow provides a zoomable and moveable view of the layers. The current
   view can
    be controlled with the mouse, the scrollbars, and a context menu.

    Plot layers are implementations of the abstract base class mpLayer. Those
   can
   ...
   vertical rulers: mpScaleX and mpScaleY.
    For convenient function plotting a series of classes derived from mpLayer
   are provided, like mpFX, mpProfile, mpLegend and so on. These base classes
   already come with plot code, user's own functions can be implemented by
   overriding just one member for retrieving a function value.

    mpWindow has built-in support for mouse-based pan and zoom through intuitive
   combinations of buttons and the mouse wheel. It also incorporates an optional
   double buffering mechanism to avoid flicker. Plots can be easily sent to
   printer evices or exported in bitmap formats like PNG, BMP or JPEG.

    @section coding Coding conventions
    wxMathPlot sticks to wxWindow's coding conventions. All entities defined by
   wxMathPlot have the prefix <i>mp</i>.
   ...
   <cdron77@users.sourceforge.net>.<br>
...
public:
  ...
  */
  ...
  /** Check whether the layer is an info box.
      The default implementation returns \a FALSE. It is overrided to \a TRUE
     for mpInfoLayer
      class and its derivative. It is necessary to define mouse actions
     behaviour over
      info boxes.
  ...
  */
  ...
  /** Plot given view of layer to the given device context.
      ...
      wxDC coordinates based on the view parameters retrievable from the
     mpWindow
      passed in \a w.
      Note that the public methods of mpWindow: x2p,y2p and p2x,p2y are already
     provided
      which transform layer coordinates to DC pixel coordinates, and <b>user
     code should rely
      on them</b> for portability and future changes to be applied
     transparently, instead of
      implementing the following formulas manually.

      The passed device context \a dc has its coordinate origin set to the
     top-left corner
      of the visible area (the default). The coordinate orientation is as shown
     in the
      following picture:
      <pre>
      (wxDC origin 0,0)
     ...
     corner of the mpWindow visible area.
      ...
      Note that Y ascends in downward direction, whereas the usual vertical
     orientation
      for mathematical plots is vice versa. Thus Y-orientation will be swapped
     usually,
      when transforming between wxDC and mpLayer coordinates. This change of
     coordinates
      ...
      dc_Y = (mpWindow::GetPosY() - layer_Y) * mpWindow::GetScaleY() // swapping
     Y-orientation

      layer_X = (dc_X / mpWindow::GetScaleX()) + mpWindow::GetPosX() // scale
     guaranteed to be not 0
      layer_Y = mpWindow::GetPosY() - (dc_Y / mpWindow::GetScaleY()) // swapping
     Y-orientation
      @endcode

      @param dc Device context to plot to.
      @param w  View to plot. The visible area can be retrieved from this
     object.
  ...
  */
  ...
  /** Set the 'continuity' property of the layer (true:draws a continuous line,
   * false:draws separate points).
    * @sa GetContinuity
    */
  ...
  /** Shows or hides the text label with the name of the layer (default is
   * visible).
    */
  ...
  */
  ...
  /** Get a small square bitmap filled with the colour of the pen used in the
     layer. Useful to create legends or similar reference to the layers.
      @param side side length in pixels
      @return a wxBitmap filled with layer's colour */
  wxBitmap GetColourSquare(int side = 16);

  /** Get layer type: a Layer can be of different types: plot lines, axis, info
     boxes, etc, this method returns the right value.
...
protected:
  ...
  bool m_continuous; //!< Specify if the layer will be plotted as a continuous
                     //line or a set of points.
  bool m_showName;   //!< States whether the name of the layer must be shown
                     //(default is true).
  bool m_drawOutsideMargins; //!< select if the layer should draw only inside
                             //margins or over all DC
...
/** @class mpInfoLayer
    ...
    mpInfoLayer is the base class to create a small rectangular info box in
   transparent overlay over plot layers. It is used to implement objects like
   legends.
*/
...
public:
  ...
  /** Updates the content of the info box. Should be overidden by derived
     classes.
      Update may behave in different ways according to the type of event which
     called it.
  ...
  /** Moves the layer rectangle of given pixel deltas.
      @param delta The wxPoint container for delta coordinates along x and y.
     Units are in pixels. */
  virtual void Move(wxPoint delta);

  /** Updates the rectangle reference point. Used by internal methods of
   * mpWindow to correctly move mpInfoLayers. */
...
/** @class mpInfoCoords
    @brief Implements an overlay box which shows the mouse coordinates in plot
   units.
    When an mpInfoCoords layer is activated, when mouse is moved over the
   mpWindow, its coordinates (in mpWindow units, not pixels) are continuously
   reported inside the layer box. */
...
public:
  ...
  /** Complete constructor, setting initial rectangle and background brush.
      ...
      @param brush The wxBrush to be used for box background: default is
     transparent */
...
/** @class mpInfoLegend
    ...
    This layer allows you to add a legend to describe the plots in the window.
   The legend uses the layer name as a label, and displays only layers of type
   mpLAYER_PLOT. */
...
/** Set label for X axis in time mode: the value is represented as
 * minutes:seconds.milliseconds if time is less than 2 minutes,
 * hours:minutes:seconds otherwise. */
#define mpX_TIME 0x01
/** Set label for X axis in hours mode: the value is always represented as
 * hours:minutes:seconds. */
#define mpX_HOURS 0x02
/** Set label for X axis in date mode: the value is always represented as
 * yyyy-mm-dd. */
#define mpX_DATE 0x03
/** Set label for X axis in datetime mode: the value is always represented as
 * yyyy-mm-ddThh:mm:ss. */
...
/** Abstract base class providing plot and labeling functionality for functions
   F:X->Y.
    Override mpFX::GetY to implement a function.
    Optionally implement a constructor and pass a name (label) and a label
   alignment
    to the constructor mpFX::mpFX. If the layer name is empty, no label will be
   plotted.
*/
class WXDLLIMPEXP_MATHPLOT mpFX : public mpLayer {
public:
  /** @param name  Label
      @param flags Label alignment, pass one of #mpALIGN_RIGHT, #mpALIGN_CENTER,
     #mpALIGN_LEFT.
  */
...
/** Abstract base class providing plot and labeling functionality for functions
   F:Y->X.
    ...
    Optionally implement a constructor and pass a name (label) and a label
   alignment
    to the constructor mpFY::mpFY. If the layer name is empty, no label will be
   plotted.
*/
class WXDLLIMPEXP_MATHPLOT mpFY : public mpLayer {
public:
  /** @param name  Label
      @param flags Label alignment, pass one of #mpALIGN_BOTTOM,
     #mpALIGN_CENTER, #mpALIGN_TOP.
  */
...
/** Abstract base class providing plot and labeling functionality for a locus
   plot F:N->X,Y.
    Locus argument N is assumed to be in range 0 .. MAX_N, and implicitly
   derived by enumerating
    all locus values. Override mpFXY::Rewind and mpFXY::GetNextXY to implement a
   locus.
    Optionally implement a constructor and pass a name (label) and a label
   alignment
    to the constructor mpFXY::mpFXY. If the layer name is empty, no label will
   be plotted.
*/
class WXDLLIMPEXP_MATHPLOT mpFXY : public mpLayer {
public:
  /** @param name  Label
      @param flags Label alignment, pass one of #mpALIGN_NE, #mpALIGN_NW,
     #mpALIGN_SW, #mpALIGN_SE.
  */
  ...
  */
  ...
  /*
    Show simbol point
  */
 virtual void ShowSimbol(bool isShow) { m_simbol=isShow;};
protected:
  int m_flags; //!< Holds label alignment
  bool m_simbol;
...
/** Abstract base class providing plot and labeling functionality for functions
   F:Y->X.
    ...
    This class is similar to mpFY, but the Plot method is different. The plot is
   in fact represented by lines instead of points, which gives best rendering of
   rapidly-varying functions, and in general, data which are not so close one to
   another.
    Optionally implement a constructor and pass a name (label) and a label
   alignment
    to the constructor mpProfile::mpProfile. If the layer name is empty, no
...
public:
  /** Full constructor.
              ...
              @param flags Set the position of the scale with respect to the
     window.
              @param ticks Select ticks or grid. Give TRUE (default) for drawing
     axis ticks, FALSE for drawing the grid.
              @param type mpX_NORMAL for normal labels, mpX_TIME for time axis
  ...
  virtual void Plot(wxDC &dc, mpWindow &w);
  // Get text label for value 
  virtual wxString GetLabelTextValue(double &v);
  /** Check whether this layer has a bounding box.
  ...
  /** Set X axis alignment.
      @param align alignment (choose between mpALIGN_BORDER_BOTTOM,
     mpALIGN_BOTTOM, mpALIGN_CENTER, mpALIGN_TOP, mpALIGN_BORDER_TOP */
  ...
  /** Get X axis label view mode.
      @return mpX_NORMAL for normal labels, mpX_TIME for time axis in hours,
     minutes, seconds. */
  unsigned int GetLabelMode() { return m_labelType; };

  /** Set X axis label view mode.
      @param mode mpX_NORMAL for normal labels, mpX_TIME for time axis in hours,
     minutes, seconds. */
...
protected:
  ...
  unsigned int m_labelType; //!< Select labels mode: mpX_NORMAL for normal
                            //labels, mpX_TIME for time axis in hours, minutes,
                            //seconds
...
/** Plot layer implementing a y-scale ruler.
    If align is set to mpALIGN_CENTER, the ruler is fixed at X=0 in the
   coordinate system. If the align is set to mpALIGN_TOP or mpALIGN_BOTTOM, the
   axis is always drawn respectively at top or bottom of the window. A label is
   plotted at
...
public:
  /** @param name Label to plot by the ruler
      ...
      @param ticks Select ticks or grid. Give TRUE (default) for drawing axis
     ticks, FALSE for drawing the grid */
  mpScaleY(wxString name = wxT("Y"), int flags = mpALIGN_CENTER,
           bool ticks = true);
  ...
  */
  ...
  /** Set Y axis alignment.
      @param align alignment (choose between mpALIGN_BORDER_LEFT, mpALIGN_LEFT,
     mpALIGN_CENTER, mpALIGN_RIGHT, mpALIGN_BORDER_RIGHT) */
  void SetAlign(int align) { m_flags = align; };
...
// WX_DECLARE_HASH_MAP( int, mpLayer*, wxIntegerHash, wxIntegerEqual,
// wxLayerList );
...
/** Canvas for plotting mpLayer implementations.
    ...
    The canvas window provides a context menu with actions for navigating the
   view.
    The context menu can be retrieved with mpWindow::GetPopupMenu, e.g. for
   extending it
    externally.
   ...
   EnableMousePanZoom.
    ...
    The mouse commands can be visualized by the user through the popup menu, and
   are:
...
public:
  ...
  */
  ...
  /** Add a plot layer to the canvas.
      @param layer Pointer to layer. The mpLayer object will get under control
     of mpWindow,
                   i.e. it will be delete'd on mpWindow destruction
      @param refreshDisplay States whether to refresh the display (UpdateAll)
  ...
  */
  ...
  /** Remove a plot layer from the canvas.
      @param layer Pointer to layer. The mpLayer object will be destructed using
     delete.
      @param alsoDeleteObject If set to true, the mpLayer object will be also
     "deleted", not just removed from the internal list.
      @param refreshDisplay States whether to refresh the display (UpdateAll)
     after removing the layer.
      @return true if layer is deleted correctly

      N.B. Only the layer reference in the mpWindow is deleted, the layer object
     still exists!
  */
  bool DelLayer(mpLayer *layer, bool alsoDeleteObject = false,
                bool refreshDisplay = true);

  /** Remove all layers from the plot.
      @param alsoDeleteObject If set to true, the mpLayer objects will be also
     "deleted", not just removed from the internal list.
      @param refreshDisplay States whether to refresh the display (UpdateAll)
  ...
  */
  ...
  /** Get current view's X position.
      See @ref mpLayer::Plot "rules for coordinate transformation"
      @return X Position in layer coordinate system, that corresponds to the
  ...
  */
  ...
  /** Get current view's Y position.
      See @ref mpLayer::Plot "rules for coordinate transformation"
      @return Y Position in layer coordinate system, that corresponds to the
     center point of the view.
  */
  ...
  */
  ...
  /** Converts mpWindow (screen) pixel coordinates into graph (floating point)
   * coordinates, using current mpWindow position and scale.
    * @sa p2y,x2p,y2p */
  //     double p2x(wxCoord pixelCoordX, bool drawOutside = true ); // { return
  //     m_posX + pixelCoordX/m_scaleX; }
   ...
   * coordinates, using current mpWindow position and scale.
    * @sa p2x,x2p,y2p */
  //     double p2y(wxCoord pixelCoordY, bool drawOutside = true ); //{ return
  //     m_posY - pixelCoordY/m_scaleY; }
  inline double p2y(wxCoord pixelCoordY) {
    return m_posY - pixelCoordY / m_scaleY;
  }

  /** Converts graph (floating point) coordinates into mpWindow (screen) pixel
   * coordinates, using current mpWindow position and scale.
    * @sa p2x,p2y,y2p */
  //     wxCoord x2p(double x, bool drawOutside = true); // { return (wxCoord) (
  //     (x-m_posX) * m_scaleX); }
  ...
  /** Enable or disable X/Y scale aspect locking for the view.
      @note Explicit calls to mpWindow::SetScaleX and mpWindow::SetScaleY will
     set
            an unlocked aspect, but any other action changing the view scale
     will
  ...
  */
  ...
  /** Set view to fit global bounding box of all plot layers and refresh
     display.
  ...
  */
  ...
  /** Set view to fit a given bounding box and refresh display.
      The X/Y scale aspect lock is taken into account.
      If provided, the parameters printSizeX and printSizeY are taken as the DC
     size, and the
      pixel scales are computed accordingly. Also, in this case the passed
     borders are not saved
      as the "desired borders", since this use will be invoked only when
     printing.
  */
  ...
  /** Zoom into current view and refresh display
    * @param centerPoint The point (pixel coordinates) that will stay in the
   * same position on the screen after the zoom (by default, the center of the
   * mpWindow).
    */
  ...
  /** Zoom view fitting given coordinates to the window (p0 and p1 do not need
  ...
  /** Counts the number of plot layers, excluding axes or text: this is to count
     only the layers which have a bounding box.
  ...
  */
  ...
  /** Returns the left-border layer coordinate that the user wants the mpWindow
   * to show (it may be not exactly the actual shown coordinate in the case of
   * locked aspect ratio).
    * @sa Fit
    */
  double GetDesiredXmin() { return m_desiredXmin; }

  /** Returns the right-border layer coordinate that the user wants the mpWindow
   * to show (it may be not exactly the actual shown coordinate in the case of
   * locked aspect ratio).
    * @sa Fit
    */
  ...
  /** Returns the bottom-border layer coordinate that the user wants the
   * mpWindow to show (it may be not exactly the actual shown coordinate in the
   * case of locked aspect ratio).
    * @sa Fit
    */
  double GetDesiredYmin() { return m_desiredYmin; }

  /** Returns the top layer-border coordinate that the user wants the mpWindow
   * to show (it may be not exactly the actual shown coordinate in the case of
   * locked aspect ratio).
    * @sa Fit
    */
  ...
  /** Returns the bounding box coordinates
          @param bbox Pointer to a 6-element double array where to store
  ...
  bool SaveScreenshot(const wxString &filename, wxBitmapType type = wxBITMAP_TYPE_BMP,
                      wxSize imageSize = wxDefaultSize, bool fit = false,bool copyboard=false);

  /** This value sets the zoom steps whenever the user clicks "Zoom in/out" or
   * performs zoom with the mouse wheel.
    *  It must be a number above unity. This number is used for zoom in, and its
   * inverse for zoom out. Set to 1.5 by default. */
  static double zoomIncrementalFactor;

  /** Set window margins, creating a blank area where some kinds of layers
     cannot draw. This is useful for example to draw axes outside the area where
     the plots are drawn.
  ...
  /** Check if a given point is inside the area of a mpInfoLayer and eventually
     returns its pointer.
...
protected:
  ...
  void
  OnSize(wxSizeEvent &event); //!< Size handler, will update scroll bar sizes
  // void OnScroll2       (wxScrollWinEvent &event); //!< Scroll handler, will
  // move canvas
  void OnShowPopupMenu(
      wxMouseEvent &event); //!< Mouse handler, will show context menu
  void OnMouseRightDown(wxMouseEvent &event); //!< Mouse handler, for detecting
  ...
  void OnMouseMove(
      wxMouseEvent &event); //!< Mouse handler for mouse motion (for pan)
  void
  ...
  int m_clickedX;  //!< Last mouse click X position, for centering and zooming
                   //the view
  int m_clickedY;  //!< Last mouse click Y position, for centering and zooming
                   //the view
  ...
  mpInfoLayer *
      m_movingInfoLayer; //!< For moving info layers over the window area
...
/** A class providing graphs functionality for a 2D plot (either continuous or a
   set of points), from vectors of data.
     This class can be used directly, the user does not need to derive any new
   class. Simply pass the data as two vectors
     with the same length containing the X and Y coordinates to the method
   SetData.
...
public:
  ...
  */
  ...
  /** Changes the internal data: the set of points to draw.
      Both vectors MUST be of the same length. This method DOES NOT refresh the
    mpWindow; do it manually.
    * @sa Clear
  */
...
public:
  mpPrintout(mpWindow *drawWindow,
...
//-----------------------------------------------------------------------------
/** This virtual class represents objects that can be moved to an arbitrary 2D
 * location+rotation.
...
public:
  /** Default constructor (sets location and rotation to (0,0,0))
    */
  mpMovableObject()
      : m_reference_x(0), m_reference_y(0), m_reference_phi(0), m_shape_xs(0),
        m_shape_ys(0) {
    m_type = mpLAYER_PLOT;
  }
  ...
  */
  ...
  /** Set label axis alignment.
    *  @param align alignment (choose between mpALIGN_NE, mpALIGN_NW,
   * mpALIGN_SW, mpALIGN_SE
    */
  void SetAlign(int align) { m_flags = align; };
...
protected:
  ...
  /** This contains the object points, in local coordinates (to be transformed
   * by the current transformation).
    */
  std::vector<double> m_shape_xs, m_shape_ys;

  /** The buffer for the translated & rotated points (to avoid recomputing them
    *with each mpWindow refresh).
    *
    */
  ...
  /** Must be called by the descendent class after updating the shape
   * (m_shape_xs/ys), or when the transformation changes.
...
/** A 2D ellipse, described by a 2x2 covariance matrix.
  ...
  *
  * The ellipse will be always centered at the origin. Use
  *mpMovableObject::SetCoordinateBase to move it.
  */
class WXDLLIMPEXP_MATHPLOT mpCovarianceEllipse : public mpMovableObject {
public:
  /** Default constructor.
    * Initializes to a unity diagonal covariance matrix, a 95% confidence
   * interval (2 sigmas), 32 segments, and a continuous plot
   * (m_continuous=true).
    */
  mpCovarianceEllipse(double cov_00 = 1, double cov_11 = 1, double cov_01 = 0,
                      double quantiles = 2, int segments = 32,
                      const wxString &layerName = wxT(""))
      : m_cov_00(cov_00), m_cov_11(cov_11), m_cov_01(cov_01),
        m_quantiles(quantiles), m_segments(segments) {
    m_continuous = true;
    m_name = layerName;
    RecalculateShape();
    m_type = mpLAYER_PLOT;
  ...
  /** Set how many "quantiles" to draw, that is, the confidence interval of the
   * ellipse (see above).
    */
  ...
  /** Returns the elements of the current covariance matrix:
    */
  void GetCovarianceMatrix(double &cov_00, double &cov_01,
                           double &cov_11) const {
    cov_00 = m_cov_00;
    cov_01 = m_cov_01;
    cov_11 = m_cov_11;
...
protected:
  /** The elements of the matrix (only 3 since cov(0,1)=cov(1,0) in any positive
   * definite matrix).
    */
...
/** An arbitrary polygon, descendant of mpMovableObject.
  *  Use "setPoints" to set the list of N points. This class also can draw
 * non-closed polygons by
  *   passing the appropriate parameters to "setPoints". To draw a point-cloud,
 * call "SetContinuity(false)".
  */
...
public:
  ...
  /** Set the points in the polygon.
    ...
    * @param closedShape If set to true, an additional segment will be added
...
public:
  ...
  /** Change the bitmap associated with the layer (to update the screen, refresh
   * the mpWindow).
    *  @param inBmp The bitmap to associate. A copy is made, thus it can be
   * released after calling this.
