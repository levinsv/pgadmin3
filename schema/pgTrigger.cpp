...
pgObject *pgTriggerFactory::CreateObjects(pgCollection *coll, ctlTree *browser, const wxString &restriction)
{
	...
	pgTrigger *trigger = 0;
	wxString ref=wxT("");
	if (collection->GetDatabase()->connection()->BackendMinimumVersion(10, 0))
	{
		ref = wxT("tgoldtable,tgnewtable,");
	}
	...
	if (triggers)
	{
		while (!triggers->Eof())
		{
			...
			if (!ref.IsEmpty()) {
				wxString strsum=wxT("");
				if (!triggers->GetVal(wxT("tgoldtable")).IsEmpty()) {
					strsum=wxT("REFERENCING OLD TABLE AS ")+triggers->GetVal(wxT("tgoldtable"));
				}
				if (!triggers->GetVal(wxT("tgnewtable")).IsEmpty()) {
					if (strsum.IsEmpty()) strsum=wxT("REFERENCING");
					strsum+=wxT(" NEW TABLE AS ")+triggers->GetVal(wxT("tgnewtable"));
				}

				trigger->iSetReferences(strsum);
			}
