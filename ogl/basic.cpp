...
void wxShapeEvtHandler::OnMoveLinks()
{
	if (m_previousHandler)
		m_previousHandler->OnMoveLinks();
}

void wxShapeEvtHandler::OnMoveLink(bool moveControlPoints)
{
	if (m_previousHandler)
		m_previousHandler->OnMoveLink(moveControlPoints);
}
...
bool wxShapeEvtHandler::OnMovePre(double x, double y, double old_x, double old_y, bool display)
{
	if (m_previousHandler)
		return m_previousHandler->OnMovePre(x, y, old_x, old_y, display);
	else
		return TRUE;
}

void wxShapeEvtHandler::OnMovePost(double x, double y, double old_x, double old_y, bool display)
{
	if (m_previousHandler)
		m_previousHandler->OnMovePost(x, y, old_x, old_y, display);
}
...
wxShape::wxShape(wxShapeCanvas *can)
{
	...
	region->SetColour(wxString(wxT("BLACK")));
}
...
void wxShape::SetDraggable(bool drag, bool recursive)
{
	...
	else if (m_sensitivity & OP_DRAG_LEFT)
...
void wxShape::Refresh(wxShapeCanvas *canvas)
{
	if (!canvas)
		canvas = m_canvas;
	if (canvas)
	{
		...
		double x1 = (double)(GetX() - w / 2.0) - canvas->GetCurrentPixelScrollPosition().x;
		double y1 = (double)(GetY() - h / 2.0) - canvas->GetCurrentPixelScrollPosition().y;
		canvas->RefreshRect(wxRect(WXROUND(x1), WXROUND(y1), WXROUND(w), WXROUND(h)));
	}
}
...
void wxShape::FormatText(wxDC &dc, const wxString &s, int i)
{
	...
	size_t j;
	for (j = 0; j < stringList->GetCount(); j++)
	{
		const wxString &s = (*stringList)[j];
	...
	region->GetSize(&actualW, &actualH);
...
void wxShape::SetTextColour(const wxColour &the_colour, int regionId)
{
	wxNode *node = m_regions.Item(regionId);
	if (!node)
		return;
...
wxColour wxShape::GetActualTextColour(int regionId) const
{
	wxNode *node = m_regions.Item(regionId);
	if (!node)
		return wxColour();
	wxShapeRegion *region = (wxShapeRegion *)node->GetData();
	return region->GetActualColourObject();
}
...
void wxShape::OnMoveLinks()
{
	...
	while (current)
	{
		...
		line->GetEventHandler()->OnMoveLink();
...
bool wxShape::OnMovePre(double WXUNUSED(x), double WXUNUSED(y), double WXUNUSED(old_x), double WXUNUSED(old_y), bool WXUNUSED(display))
{
	return TRUE;
}

void wxShape::OnMovePost(double WXUNUSED(x), double WXUNUSED(y), double WXUNUSED(old_x), double WXUNUSED(old_y), bool WXUNUSED(display))
{
}
...
bool wxShape::MoveLineToNewAttachment(wxLineShape *to_move,
                                      double x, double y)
{
...
void wxShape::OnDrawOutline(wxDC &dc, double x, double y, double w, double h)
{
	...
	wxPoint points[5];
	points[0].x = WXROUND(top_left_x);
	points[0].y = WXROUND(top_left_y);
	points[1].x = WXROUND(top_right_x);
	points[1].y = WXROUND(top_right_y);
	points[2].x = WXROUND(bottom_right_x);
	points[2].y = WXROUND(bottom_right_y);
	points[3].x = WXROUND(bottom_left_x);
	points[3].y = WXROUND(bottom_left_y);
	points[4].x = WXROUND(top_left_x);
	points[4].y = WXROUND(top_left_y);
...
void wxShape::Move(double x, double y, bool display)
{
	double old_x = m_xpos;
	double old_y = m_ypos;

	if (!GetEventHandler()->OnMovePre(x, y, old_x, old_y, display))
	{
//    m_xpos = old_x;
//    m_ypos = old_y;
		return;
	...
	GetEventHandler()->OnMovePost(x, y, old_x, old_y, display);
}

void wxShape::MoveLinks()
{
	GetEventHandler()->OnMoveLinks();
}
...
void wxShape::ResetControlPoints()
{
	...
	control->m_xoffset = left;
	control->m_yoffset = top;

	node = node->GetNext();
	control = (wxControlPoint *)node->GetData();
	control->m_xoffset = 0;
	control->m_yoffset = top;

	node = node->GetNext();
	control = (wxControlPoint *)node->GetData();
	control->m_xoffset = right;
	control->m_yoffset = top;
...
void wxShape::Select(bool select)
{
	m_selected = select;
	if (select)
	{
