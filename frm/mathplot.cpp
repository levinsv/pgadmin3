  ...
  if (event.GetEventType() == wxEVT_MOTION) {
    ...
    if ((ty!=tx) && (tx!=0)) {
        if (lastx != tx && lasty != ty) {
            ...
            mpScaleX* xaxis = ((mpScaleX*)(w.GetLayer(0)));
            mpScaleY* yaxis = ((mpScaleY*)(w.GetLayer(1)));
            //double x=w.p2x(sx);
            double x = tx;
            ...
            m_content = toolTipContent;
            //wxTipWindow* tip = new wxTipWindow(w.GetParent(), toolTipContent, 100, ptr,&rectBounds);
            lastx = tx; lasty = ty;
            mouseX = sx;
            mouseY = sy;
        }
...
#else
  //  m_content.Printf(wxT("x = %f\ny = %f\nD=%f"), w.p2x(mouseX), w.p2y(mouseY),dd);
#endif
  }
}
...
void mpInfoLegend::UpdateInfo(mpWindow &w, wxEvent &event) {
    //wxMouseEvent mv = &event;
    wxPoint pointClicked = ((wxMouseEvent&)event).GetPosition();
    mpInfoLayer* m_movingInfoLayer = w.IsInsideInfoLayer(pointClicked);
    if (m_movingInfoLayer != NULL) {
        ///((mpInfoLegend&)m_movingInfoLayer).m_mouse_point = pointClicked;
        m_mouse_point = pointClicked;
    }
}
...
// label.c_str());
    ...
    if (textY != mpLEGEND_MARGIN) { // Don't draw any thing if there are no
      ...
      int plotCount = 0;
      int posY = 0;
      wxPoint pp(m_mouse_point.x - m_dim.x, m_mouse_point.y - m_dim.y);
      for (unsigned int p2 = 0; p2 < w.CountAllLayers(); p2++) {
        ...
        if ((ly->GetLayerType() == mpLAYER_PLOT) && (ly->IsVisible())) {
          ...
          wxRect r;
          bool ss = ly->IsSelect();
          r.x = 0; r.y = posY- m_dim.y; r.width = m_dim.width; r.height= tmpY;
          if (r.Contains(pp)) {
              wxPen pensel(lpen);
              pensel.SetWidth(lpen.GetWidth() * 2);
              dc.SetPen(pensel);
              ly->SetSelect(true);
          } else ly->SetSelect(false);
          if (ss != ly->IsSelect()) {
              w.Refresh();
          }
          ...
          dc.DrawText(label, m_dim.x + baseWidth,
...
void mpFX::Plot(wxDC &dc, mpWindow &w) {
  if (m_visible) {
    ...
    wxCoord endPx =
        m_drawOutsideMargins ? w.GetScrX() : w.GetScrX() - w.GetMarginRight();
    ...
    if (m_pen.GetWidth() <= 1) {
      for (wxCoord i = startPx; i < endPx; ++i) {
        ...
        if (m_drawOutsideMargins || ((iy >= minYpx) && (iy <= maxYpx)))
          dc.DrawPoint(i, iy); // (wxCoord) ((w.GetPosY() - GetY( (double)i /
                               // w.GetScaleX() + w.GetPosX()) ) *
                               // w.GetScaleY()));
      }
    } else {
      for (wxCoord i = startPx; i < endPx; ++i) {
    ...
    if (!m_name.IsEmpty() && m_showName) {
      ...
      */
      ...
      dc.DrawText(m_name, tx,
                  w.y2p(GetY(w.p2x(tx)))); // (wxCoord) ((w.GetPosY() - GetY(
                                           // (double)tx / w.GetScaleX() +
                                           // w.GetPosX())) * w.GetScaleY()) );
    }
  }
}
  ...
  if (m_visible) {
    ...
    if (m_pen.GetWidth() <= 1) {
      ...
      for (i = 0; i < w.GetScrY(); ++i) {
        ...
        //             wxCoord c =  w.x2p(GetX(w.p2y(i))); //(wxCoord) ((GetX(
        //             (double)i / w.GetScaleY() + w.GetPosY()) - w.GetPosX()) *
        //             w.GetScaleX());
        //             dc.DrawLine(c, i, c, i);
      }
    }
    ...
    if (!m_name.IsEmpty() && m_showName) {
      ...
      dc.DrawText(m_name, w.x2p(GetX(w.p2y(ty))),
      ...
      while (GetNextXY(x, y)) {
        ...
        if ((x1 >= startPx) && (x0 <= endPx)) {
          ...
          if (!outUp && !outDown) {
            if (c1 != c0) {
              if (c0 < minYpx) {
                x0 = (int)(((float)(minYpx - c0)) / ((float)(c1 - c0)) *
                           (x1 - x0)) +
                     x0;
                c0 = minYpx;
              }
              if (c0 > maxYpx) {
                x0 = (int)(((float)(maxYpx - c0)) / ((float)(c1 - c0)) *
                           (x1 - x0)) +
                     x0;
              ...
              if (c1 < minYpx) {
                x1 = (int)(((float)(minYpx - c0)) / ((float)(c1 - c0)) *
                           (x1 - x0)) +
                     x0;
                c1 = minYpx;
              }
              if (c1 > maxYpx) {
                x1 = (int)(((float)(maxYpx - c0)) / ((float)(c1 - c0)) *
                           (x1 - x0)) +
                     x0;
            ...
            if (x1 != x0) {
              if (x0 < startPx) {
                c0 = (int)(((float)(startPx - x0)) / ((float)(x1 - x0)) *
                           (c1 - c0)) +
                     c0;
                x0 = startPx;
              }
              if (x1 > endPx) {
                c1 = (int)(((float)(endPx - x0)) / ((float)(x1 - x0)) *
                           (c1 - c0)) +
                     c0;
                x1 = endPx;
              }
            ...
            if (m_simbol) dc.DrawRectangle(x1 - wsimb*2 / 2, c1 - wsimb*2 / 2, wsimb*2, wsimb*2);
...
void mpProfile::Plot(wxDC &dc, mpWindow &w) {
  if (m_visible) {
    ...
    for (wxCoord i = startPx; i < endPx; ++i) {
      wxCoord c0 = w.y2p(GetY(w.p2x(i)));     // (wxCoord) ((w.GetYpos() - GetY(
                                              // (double)i / w.GetXscl() +
                                              // w.GetXpos()) ) * w.GetYscl());
      wxCoord c1 = w.y2p(GetY(w.p2x(i + 1))); //(wxCoord) ((w.GetYpos() - GetY(
    ...
    if (!m_name.IsEmpty()) {
      ...
      dc.DrawText(m_name, tx, w.y2p(GetY(w.p2x(tx)))); //(wxCoord) ((w.GetPosY()
                                                       //- GetY( (double)tx /
      // w.GetScaleX() +
      // w.GetPosX())) *
      // w.GetScaleY()) );
...
mpScaleX::mpScaleX(wxString name, int flags, bool ticks, unsigned int type) {
  ...
  SetPen(DEFAULT_AXIS_TICK_COLOUR);
...
wxString mpScaleX::GetLabelTextValue(double &v) {
    wxCoord tx, ty;
    wxString s;
    wxString fmt;
    int tmp = 2;
    if (m_labelType == mpX_NORMAL) {
      ...
      if (m_labelType == mpX_DATETIME) {
        ...
        if (m_labelType == mpX_NORMAL)
          ...
          time_t when = (time_t) v;
          struct tm tm = *localtime(&when);
          s.Printf(fmt, (double)tm.tm_year + 1900, (double)tm.tm_mon + 1,
                   (double)tm.tm_mday, (double)tm.tm_hour, (double)tm.tm_min,
                   (double)tm.tm_sec);
        } else if (m_labelType == mpX_DATE) {
          time_t when = (time_t) v;
          struct tm tm = *localtime(&when);
          ...
          double modulus = fabs(v);
          double sign = v / modulus;
          double hh = floor(modulus / 3600);
          double mm = floor((modulus - hh * 3600) / 60);
          double ss = modulus - hh * 3600 - mm * 60;
#ifdef MATHPLOT_DO_LOGGING
          wxLogMessage(wxT("%02.0f Hours, %02.0f minutes, %02.0f seconds"),
                       sign * hh, mm, ss);
...
void mpScaleX::Plot(wxDC &dc, mpWindow &w) {
  if (m_visible) {
    ...
    /*if (m_drawOutsideMargins == true)
        ...
        dc.DrawLine( w.GetMarginLeft(), orgy, w.GetScrX() - w.GetMarginRight(),
    orgy); */
    ...
    for (n = n0; n < end; n += step) {
      ...
      if ((p >= startPx) && (p <= endPx)) {
        ...
        dc.GetMultiLineTextExtent(s, &tx, &ty);
        labelH = (labelH <= ty) ? ty : labelH;
        /*				if ((p-tx/2-tmp) > 64) { // Problem
           about
           non-regular axis labels
                                                if ((m_flags ==
           mpALIGN_BORDER_BOTTOM) || (m_flags == mpALIGN_TOP)) {
                                                        dc.DrawText( s, p-tx/2,
           orgy-4-ty);
                                                } else {
                                                        dc.DrawText( s, p-tx/2,
           orgy+4);
                                        ...
                                        */
        maxExtent =
            (tx > maxExtent) ? tx : maxExtent; // Keep in mind max label width
      }
    }
    // Actually draw labels, taking care of not overlapping them, and
    // distributing them regularly
    double labelStep = ceil((maxExtent + mpMIN_X_AXIS_LABEL_SEPARATION) /
                            (w.GetScaleX() * step)) *
                       step;
    ...
    case mpALIGN_TOP: {
      if ((!m_drawOutsideMargins) && (w.GetMarginTop() > (ty + labelH + 8))) {
        dc.DrawText(m_name, (endPx - startPx - tx) >> 1,
                    orgy - 6 - ty - labelH);
      } else {
  ...
  if (m_visible) {
    ...
    for (; n < end; n += step) {
      ...
      if ((p >= minYpx) && (p <= maxYpx)) {
        ...
        if (ty != labelHeigth)
          wxLogMessage(
              wxT("mpScaleY::Plot: ty(%f) and labelHeigth(%f) differ!"), ty,
              labelHeigth);
#endif
    ...
    case mpALIGN_LEFT: {
      if ((!m_drawOutsideMargins) && (w.GetMarginLeft() > (ty + labelW + 8))) {
        dc.DrawRotatedText(m_name, orgx - 6 - labelW - ty,
    ...
    case mpALIGN_RIGHT: {
      if ((!m_drawOutsideMargins) && (w.GetMarginRight() > (ty + labelW + 8))) {
        dc.DrawRotatedText(m_name, orgx + 6 + labelW,
                           (maxYpx - minYpx + tx) >> 1, 90);
      } else {
...
mpWindow::mpWindow(wxWindow *parent, wxWindowID id, const wxPoint &pos,
  ...
  m_popmenu.Append(mpID_CENTER, _("Center"),
  ...
  m_popmenu.AppendCheckItem(mpID_LOCKASPECT, _("Lock aspect"),
                            _("Lock horizontal and vertical zoom aspect."));
  m_popmenu.Append(mpID_HELP_MOUSE, _("Show mouse commands..."),
...
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage(_("[mpWindow::OnMouseMove] Ax:%i Ay:%i m_posX:%f m_posY:%f"),
                 Ax, Ay, m_posX, m_posY);
#endif
  } else {
    if (event.m_leftDown) {
      if (m_movingInfoLayer == NULL) {
        ...
        dc.DrawRectangle(m_mouseLClick_X, m_mouseLClick_Y,
                         event.GetX() - m_mouseLClick_X,
                         event.GetY() - m_mouseLClick_Y);
      } else {
        wxPoint moveVector(event.GetX() - m_mouseLClick_X,
                           event.GetY() - m_mouseLClick_Y);
...
void mpWindow::Fit(double xMin, double xMax, double yMin, double yMax,
  ...
  if (m_lockaspect) {
#ifdef MATHPLOT_DO_LOGGING
    wxLogMessage(_("mpWindow::Fit()(lock) m_scaleX=%f,m_scaleY=%f"), m_scaleX,
                 m_scaleY);
#endif
    // Keep the lowest "scale" to fit the whole range required by that axis (to
    // actually "fit"!):
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::Fit() m_desiredXmin=%f m_desiredXmax=%f  "
                 "m_desiredYmin=%f m_desiredYmax=%f"),
               xMin, xMax, yMin, yMax);
  wxLogMessage(_("mpWindow::Fit() m_scaleX = %f , m_scrX = %d,m_scrY=%d, "
                 "Ax=%f, Ay=%f, m_posX=%f, m_posY=%f"),
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::DoZoomInXCalc() prior X coord: (%f), new X coord: "
                 "(%f) SHOULD BE EQUAL!!"),
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::DoZoomInYCalc() prior Y coord: (%f), new Y coord: "
                 "(%f) SHOULD BE EQUAL!!"),
...
void mpWindow::DoZoomOutXCalc(const int staticXpixel) {
  ...
  m_desiredXmax = m_posX + (m_scrX - (m_marginLeft + m_marginRight)) / m_scaleX;
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::DoZoomOutXCalc() prior X coord: (%f), new X coord: "
                 "(%f) SHOULD BE EQUAL!!"),
               staticX, p2x(staticXpixel));
#endif
}
...
void mpWindow::DoZoomOutYCalc(const int staticYpixel) {
  ...
  m_desiredYmin = m_posY - (m_scrY - (m_marginTop + m_marginBottom)) / m_scaleY;
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::DoZoomOutYCalc() prior Y coord: (%f), new Y coord: "
                 "(%f) SHOULD BE EQUAL!!"),
               staticY, p2y(staticYpixel));
#endif
}
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::ZoomOut() prior coords: (%f,%f), new coords: "
                 "(%f,%f) SHOULD BE EQUAL!!"),
               prior_layer_x, prior_layer_y, p2x(c.x), p2y(c.y));
#endif
  UpdateAll();
...
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("Zoom: (%f,%f)-(%f,%f)"), zoom_x_min, zoom_y_min, zoom_x_max,
               zoom_y_max);
#endif
...
void mpWindow::OnSize(wxSizeEvent &WXUNUSED(event)) {
  ...
  Fit(m_desiredXmin, m_desiredXmax, m_desiredYmin, m_desiredYmax);
#ifdef MATHPLOT_DO_LOGGING
  wxLogMessage(_("mpWindow::OnSize() m_scrX = %d, m_scrY = %d"), m_scrX,
               m_scrY);
#endif // MATHPLOT_DO_LOGGING
}
...
bool mpWindow::DelLayer(mpLayer *layer, bool alsoDeleteObject,
                        bool refreshDisplay) {
...
//     DoPrepareDC(dc);
...
#ifdef MATHPLOT_DO_LOGGING
  {
    int px, py;
    GetViewStart(&px, &py);
    wxLogMessage(_("[mpWindow::OnPaint] vis.area:%ix%i px=%i py=%i"), m_scrX,
                 m_scrY, px, py);
  }
#endif
  ...
  if (UpdateBBox()) {
    if (m_enableScrollBars) {
      ...
      // Do x scroll bar
      {
        ...
        SetScrollbar(wxHORIZONTAL, thumbX, cx - (m_marginRight + m_marginLeft),
                     sizeX);
      }
      // Do y scroll bar
      {
        ...
        SetScrollbar(wxVERTICAL, thumbY, cy - (m_marginTop + m_marginBottom),
                     sizeY);
      }
    }
  }
...
bool mpWindow::SaveScreenshot(const wxString &filename, wxBitmapType type,
                              wxSize imageSize, bool fit, bool copyboard) {
  int sizeX, sizeY;
  int bk_scrX, bk_scrY;
  ...
  if (imageSize != wxDefaultSize) {
    ...
    Fit(m_desiredXmin, m_desiredXmax, m_desiredYmin, m_desiredYmax, &bk_scrX,
        &bk_scrY);
    UpdateAll();
  }
...
double mpFXYVector::GetDistance(double &x,double &y) {
    ...
    double minD=1e38;
    bool first=true;
    for (itX = m_xs.begin(),itY = m_ys.begin(); itX != m_xs.end(); ++itX,++itY) {
      x1=*itX;
      y1=*itY;
      double d=sqrt((x1-tx)*(x1-tx)+(y1-ty)*(y1-ty));
      if (first) {minD=d; x=x1;y=y1;}else
          if (minD>d) {minD=d;x=x1;y=y1;}
      first=false;
    }
    return minD;
...
bool mpPrintout::OnPrintPage(int page) {
  ...
  if ((trgDc) && (page == 1)) {
    ...
    plotWindow->Fit(plotWindow->GetDesiredXmin(), plotWindow->GetDesiredXmax(),
                    plotWindow->GetDesiredYmin(), plotWindow->GetDesiredYmax(),
                    &m_prnX, &m_prnY);
...
//-----------------------------------------------------------------------------
void mpMovableObject::TranslatePoint(double x, double y, double &out_x,
                                     double &out_y) {
...
void mpBitmapLayer::SetBitmap(const wxImage &inBmp, double x, double y,
                              double lx, double ly) {
  if (!inBmp.Ok()) {
    ...
    */
    ...
    if (d_width > 0 && d_height > 0) {
      ...
      if (m_scaledBitmap.GetWidth() != d_width ||
        ...
        m_scaledBitmap =
            wxBitmap(wxBitmap(m_bitmap).GetSubBitmap(r).ConvertToImage().Scale(
                d_width, d_height));
        m_scaledBitmap_offset_x = offset_x;
        m_scaledBitmap_offset_y = offset_y;
      }
